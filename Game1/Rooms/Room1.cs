...
namespace Game1.Rooms
{
    class Room1 : Room
    {
        ...
        public override void Draw()
        {
            ...
            DrawTextOutlined(Arial24, "xXx_PUSSYSLAYER69_xXx", new Vector2(180, 256), Color.White, Color.Black);
            DrawTextOutlined(Arial24, "DePiemelMan", new Vector2(180, 416), Color.White, Color.Black);
            DrawTextOutlined(Arial24, "CrazyCreeper", new Vector2(180, 576), Color.White, Color.Black);
        ...
        private void DrawTextOutlined(SpriteFont font, object text, Vector2 position, Color textColor, Color outlineColor, int borderWidth = 1)
        {
            for (int x = -borderWidth; x <= borderWidth; x++)
            {
                ...
                {
                    View.DrawText(font, text, position + new Vector2(x, y), outlineColor, 0, new Vector2(0, font.MeasureString("Aj").Y / 2));
                }
            }
            View.DrawText(font, text, position, textColor, 0, new Vector2(0, font.MeasureString("Aj").Y / 2));
        }
    }
}
